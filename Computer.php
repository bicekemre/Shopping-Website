...
$Query				                        =	$DatabaseConnect->prepare("SELECT * FROM Items WHERE ItemType = 'Computer' AND Status = '1' $ConditionforMenu $ConditionforSearch ORDER BY id DESC");
$Query->execute();
...
$MainPageItemsQuery		=	$DatabaseConnect->prepare("SELECT SUM(ItemAmount) AS MenusItems FROM menus WHERE ItemType = 'Computer'");
...
<table width="1065" align="center" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td><table width="450" align="center" border="0" cellpadding="0" cellspacing="0">
                <tr align="left" height="30"><a href="index.php?PageCode=79" style="text-decoration: none; <?php if($IncomingMenuId==""){ ?>color: black;<? }else{ ?>color: #646464;<?php } ?> font-weight: bold;">&nbsp;Items (<?php echo $MainPageItemsQuery["MenusItems"]; ?>)</a></tr>

                <?php
                $MenusQuery		=	$DatabaseConnect->prepare("SELECT * FROM menus WHERE ItemType = 'Computer' ORDER BY MenuName ASC");
                $MenusQuery->execute();
                ...
                ?>
                <td height="5">
                <tr><a href="index.php?PageCode=79&MenuID=<?php echo $Menu["id"]; ?>" style="text-decoration: none; <?php if($IncomingMenuId==$Menu["id"]){ ?>color: black;<? }else{ ?>color: #646464;<?php } ?> font-weight: bold;">&nbsp;<?php echo FiltersDecode($Menu["MenuName"]); ?> (<?php echo FiltersDecode($Menu["ItemAmount"]); ?>)</a></tr>
                </td>
                <?php
                }
                ?>
                <tr>
                    <td><div class="SearchArea"><form action="index.php?PageCode=79" method="post">
                <?php
                if($IncomingMenuId!=""){
                ?>
                ...
                <tr><?php
                        $ItemsQuery		=	$DatabaseConnect->prepare("SELECT * FROM items WHERE ItemType = 'Computer' AND Status = '1' $ConditionforMenu $ConditionforSearch ORDER BY id DESC LIMIT $offset, $ViewingDataCountinperPage");
                        $ItemsQuery->execute();
                        ...
                        <td width="191" valign="top">
                            <table width="191" align="left" border="0" cellpadding="0" cellspacing="0" style="margin-bottom: 10px;"> <!-- border: 1px solid #CCCCCC; -->
                                <tr height="40">
                                    <td align="center"><a href="index.php?PageCode=77&ID=<?php echo FiltersDecode($Records["id"]); ?>"><img src="Images\ItemPictures\Computer/<?php echo FiltersDecode($Records["ItemPicOne"]); ?>" border="0" width="225" height="255"></a></td>
                                </tr>
                                <tr height="25">
                                    <td width="191" align="center"><a href="index.php?PageCode=77&ID=<?php echo FiltersDecode($Records["id"]); ?>" style="color: black; font-weight: bold; text-decoration: none;">Computer</a></td>
                                </tr>
                                <tr height="25">
